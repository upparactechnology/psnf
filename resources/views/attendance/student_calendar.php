...
$breadcrumbs = [['label' => 'Dashboard', 'url' => '/dashboard'], ['label' => 'Attendance', 'url' => '/academics/attendance'], ['label' => 'Calendar']];
ob_start();
?>
...
<div class="space-y-6 max-w-6xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Student Attendance Calendar</h1>
            <p class="text-xs text-slate-500 mt-0.5">Visualize monthly attendance records for students.</p>
        </div>
        <div class="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl shadow-inner">
            <a href="<?= url('academics/attendance') ?>" class="px-4 py-2 rounded-lg text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">Daily List</a>
            <a href="<?= url('academics/attendance?view=calendar') ?>" class="px-4 py-2 rounded-lg text-xs font-semibold bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow shadow-slate-200/50 dark:shadow-none">Monthly Calendar</a>
        </div>
    </div>

    <!-- Filters -->
    <form action="<?= url('academics/attendance') ?>" method="GET" class="p-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 flex flex-wrap items-center justify-between gap-4 text-xs">
        <input type="hidden" name="view" value="calendar">
        <div class="flex flex-wrap items-center gap-3">
            <div class="flex items-center gap-2">
                <span class="font-bold text-slate-700 dark:text-slate-300">Month:</span>
            ...
            <div class="flex items-center gap-2">
                <span class="font-bold text-slate-700 dark:text-slate-300">Class:</span>
                <select name="class" class="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-1.5 text-slate-900 dark:text-white">
                    <?php foreach ($classes as $c): ?>
                        <option value="<?= e($c['class']) ?>" <?= $selectedClass === $c['class'] ? 'selected' : '' ?>>
                            <?= e($c['class'] . ($c['section'] ? ' - ' . $c['section'] : '')) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="flex items-center gap-2">
                <span class="font-bold text-slate-700 dark:text-slate-300">Student:</span>
                <select name="student_id" class="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-1.5 text-slate-900 dark:text-white max-w-[200px]">
                    <option value="0">All Students</option>
                    <?php foreach ($students as $s): ?>
                        <option value="<?= $s['id'] ?>" <?= (int)$studentId === (int)$s['id'] ? 'selected' : '' ?>>
    ...
    <div class="p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40">
        <div class="mb-6 flex items-center justify-between">
            <h3 class="text-lg font-bold text-slate-900 dark:text-white">
                <?= e($selectedStudent['first_name'] . ' ' . $selectedStudent['last_name']) ?>'s Attendance 
                <span class="text-sm font-normal text-slate-500 ml-2">(<?= $monthName ?>)</span>
            </h3>
    ...
    <?php else: ?>
    <div class="p-8 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 text-center">
        <p class="text-sm text-slate-500">Select a student to view their attendance calendar.</p>
    </div>
    <?php endif; ?>
