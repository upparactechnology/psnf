...
$breadcrumbs = [
    ...
    ['label' => 'Exams Setup', 'url' => '/academics/exams?academic_year_id=' . ($exam['academic_year_id'] ?? '') . '&main_group_id=' . ($exam['main_group_id'] ?? '') . '&semester=' . urlencode($exam['semester'] ?? '')],
    ['label' => 'Edit Exam']
];
$existingSubjectIds = json_decode($exam['subject_ids'] ?? 'null', true) ?: [];
ob_start();
?>

<div x-data="editExam()" x-init="fetchSubjects()" class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl border border-slate-800/60 bg-slate-900/40 backdrop-blur">
        <div>
            <h2 class="text-xl font-bold text-white">Edit Exam</h2>
            <p class="text-sm text-slate-500 mt-0.5">Update exam details and subject assignments</p>
        </div>
        <a href="<?= url('academics/exams?academic_year_id=' . ($exam['academic_year_id'] ?? '') . '&main_group_id=' . ($exam['main_group_id'] ?? '') . '&semester=' . urlencode($exam['semester'] ?? '')) ?>"
    ...
    <div class="rounded-2xl border border-slate-800/60 bg-slate-900/40 p-6 shadow-sm">
        <form method="POST" action="<?= url('academics/exams/' . $exam['id'] . '/update') ?>" class="space-y-5">
            <?= \Core\View::csrf() ?>
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" name="name" value="<?= e($exam['name']) ?>" required placeholder="e.g. Unit Test - 1"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div class="space-y-1.5">
                    ...
                    <select name="academic_year_id" required
                        ...
                        <?php foreach ($years as $y): ?>
                            <option value="<?= $y['id'] ?>" <?= ($exam['academic_year_id'] == $y['id']) ? 'selected' : '' ?>><?= e($y['year_name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="semester" required
                            class="w-full bg-slate-950 border border-slate-800 text-slate-300 rounded-xl py-2.5 px-3 text-sm focus:outline-none focus:border-brand-500 transition-all">
                        <option value="Semester 1" <?= $exam['semester'] === 'Semester 1' ? 'selected' : '' ?>>Semester 1</option>
                        <option value="Semester 2" <?= $exam['semester'] === 'Semester 2' ? 'selected' : '' ?>>Semester 2</option>
                    </select>
                </div>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <input type="number" step="0.01" name="max_marks" value="<?= e($exam['max_marks']) ?>" required
                        ...
                        <div class="space-y-1">
                            <template x-for="subj in curriculumSubjects" :key="subj.id">
                                <label class="flex items-center gap-2 cursor-pointer hover:bg-slate-900 rounded px-2 py-1.5">
                                    <input type="checkbox" name="subject_ids[]" :value="String(subj.id)" x-model="selectedSubjectIds"
...
function editExam() {
    return {
        selectedGroupId: <?= $exam['main_group_id'] ?? 0 ?>,
        selectedYearId: <?= $exam['academic_year_id'] ?? 0 ?>,
        selectedSubjectIds: <?= json_encode(array_map('strval', $existingSubjectIds)) ?>,
        curriculumSubjects: [],
