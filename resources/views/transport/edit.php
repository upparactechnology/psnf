<?php
$layout    = 'app';
$pageTitle = 'Edit Transport Route';
$breadcrumbs = [
...
<div class="max-w-2xl mx-auto">
    <div class="rounded-2xl border border-slate-200 dark:border-slate-800/60 bg-white dark:bg-slate-900/30 backdrop-blur p-6 shadow-sm">
        <h3 class="text-base font-bold text-slate-900 dark:text-white mb-6">Edit Bus Route</h3>

        <form action="<?= url("transport/{$route['id']}") ?>" method="POST" class="space-y-5">
            <?= \Core\View::csrf() ?>
            ...
            <div>
                ...
                <input type="text" name="route_name" required value="<?= e($route['route_name']) ?>"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="bus_number" required value="<?= e($route['bus_number']) ?>"
                ...
                <div>
                    ...
                    <select name="status" required
                        ...
                        <option value="inactive" <?= $route['status'] === 'inactive' ? 'selected' : '' ?>>Inactive / Rest</option>
                        <option value="en_route" <?= $route['status'] === 'en_route' ? 'selected' : '' ?>>En Route (Active)</option>
            ...
            <div>
                <label class="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5 font-sans">Select Registered Driver (Optional)</label>
                <select id="driver_select" onchange="fillDriverDetails(this)"
                        class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-white rounded-xl py-2.5 px-3.5 text-sm focus:outline-none focus:border-brand-500 transition-all font-sans">
                    <option value="">-- Choose Driver User --</option>
                    <?php foreach ($drivers as $d): ?>
                    <option value="<?= e($d['name']) ?>" data-phone="<?= e($d['phone']) ?>" <?= $route['driver_name'] === $d['name'] ? 'selected' : '' ?>><?= e($d['name']) ?> (<?= e($d['email']) ?>)</option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="driver_name" required value="<?= e($route['driver_name']) ?>"
                           class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-white rounded-xl py-2.5 px-3.5 text-sm focus:outline-none focus:border-brand-500 transition-all">
                ...
                <div>
                    ...
                    <input type="text" name="driver_phone" required value="<?= e($route['driver_phone']) ?>"
                           class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-white rounded-xl py-2.5 px-3.5 text-sm focus:outline-none focus:border-brand-500 transition-all">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2 border-t border-slate-100 dark:border-slate-800/80">
                <div>
                    ...
                    <input type="number" step="0.000001" name="current_latitude" value="<?= e($route['current_latitude'] ?? '') ?>"
                           class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-white rounded-xl py-2.5 px-3.5 text-sm focus:outline-none focus:border-brand-500 transition-all font-mono">
                </div>
                <div>
                    ...
                    <input type="number" step="0.000001" name="current_longitude" value="<?= e($route['current_longitude'] ?? '') ?>"
                           class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-white rounded-xl py-2.5 px-3.5 text-sm focus:outline-none focus:border-brand-500 transition-all font-mono">
                </div>
            </div>
