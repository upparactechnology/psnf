...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <?php $workingDaysMap = json_decode($shift['working_days_json'] ?? '{}', true) ?: []; ?>
    <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 overflow-hidden text-xs">
        ...
        <form action="<?= url('payroll/settings/save') ?>" method="POST" class="p-6 space-y-4">
            <?= \Core\View::csrf() ?>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                ...
                <?php
                    ...
                    for ($d = 1; $d <= $daysInThisMonth; $d++) {
                        ...
                        if ($w != 0) $weekdays++; // Mon-Sat
                    }
                    $saved = (int)($workingDaysMap[$monthKey] ?? 0);
                ?>
            ...
            <div class="p-3 rounded-xl bg-indigo-50/50 dark:bg-indigo-950/20 border border-indigo-200 dark:border-indigo-800/30 flex items-start gap-2">
                ...
                <div class="text-3xs text-indigo-700 dark:text-indigo-300 leading-relaxed">
                    <strong>How it works:</strong> Leave a field empty or set to 0 to auto-calculate as all weekdays (Mon-Sat) in that month. The value you set here will be used for <strong>all</strong> salary calculations, attendance summaries, and payroll processing. Set to 0 to auto-calculate.
                </div>
            </div>
            
            <div class="flex justify-end gap-2">
                <button type="submit" class="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 font-semibold text-white transition-all shadow-sm flex items-center gap-2">
