...
<div class="max-w-4xl mx-auto space-y-5">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-0.5">Manage organization details, school information, and receipt customizations</p>
    </div>

    <form method="POST" action="<?= url('settings/update') ?>" enctype="multipart/form-data" x-data="{ loading: false }" @submit="loading = true">
        <?= \Core\View::csrf() ?>
        <input type="hidden" name="redirect_tab" value="<?= url('settings/general') ?>">
        ...
        <div class="rounded-2xl border border-slate-800/60 bg-slate-900/40 p-6 space-y-4 mb-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-3 border-t border-slate-800/40">
                <div class="space-y-1.5">
                    ...
                    <input type="url" name="school_website" placeholder="https://example.com" value="<?= e($school['website'] ?? '') ?>"
                           class="w-full bg-slate-900/70 border border-slate-700/60 text-white rounded-xl py-2.5 px-4 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30 transition-all">
                </div>
                <div class="space-y-1.5">
                    <label class="block text-xs font-medium text-slate-400">Principal Name</label>
                    <input type="text" name="school_principal" placeholder="Dr. Principal Name" value="<?= e($school['principal'] ?? '') ?>"
                           class="w-full bg-slate-900/70 border border-slate-700/60 text-white rounded-xl py-2.5 px-4 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30 transition-all">
                </div>
                <div class="space-y-1.5">
                    <label class="block text-xs font-medium text-slate-400">Affiliation Registration Code</label>
                    <input type="text" name="school_affiliation" placeholder="REG-87429" value="<?= e($school['affiliation'] ?? '') ?>"
                           class="w-full bg-slate-900/70 border border-slate-700/60 text-white rounded-xl py-2.5 px-4 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500/30 transition-all">
                </div>
            </div>
        </div>
