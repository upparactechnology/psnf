<?php
$layout    = 'app';
$pageTitle = 'Parent Document Registry';
$breadcrumbs = [['label' => 'Dashboard', 'url' => '/dashboard'], ['label' => 'Documents Workspace', 'url' => '/documents'], ['label' => 'Parent Documents']];
ob_start();
?>
...
<div class="max-w-6xl mx-auto py-2">
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-1 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 p-4 space-y-4">
            <h3 class="font-bold text-slate-800 dark:text-white text-xs uppercase tracking-wider">Parents Registry</h3>
            
            <div class="relative">
                <span class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </span>
                <input type="text" id="parentSearch" placeholder="Search parent name..." 
            ...
            <div class="space-y-1 overflow-y-auto max-h-[480px] pr-1" id="parentList">
                <?php foreach ($parents as $p): ?>
                <a href="?parent_id=<?= $p['id'] ?>" 
                   class="block p-2.5 rounded-xl text-xs transition-all <?= $p['id'] == $parentId ? 'bg-emerald-600 text-white font-bold' : 'hover:bg-slate-50 dark:hover:bg-slate-800/40 text-slate-700 dark:text-slate-300' ?>">
                    <div class="truncate"><?= e($p['name']) ?></div>
                    <div class="text-[10px] opacity-80 font-mono mt-0.5"><?= e($p['email']) ?> · <?= e($p['phone_number']) ?></div>
                </a>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="lg:col-span-3 space-y-6">
            ...
            <div class="p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 flex items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-emerald-500/10 text-emerald-500 flex items-center justify-center text-xl font-bold font-mono">
                        <?= strtoupper(substr($activeParent['name'], 0, 1)) ?>
                    </div>
                    <div>
                        <h2 class="text-lg font-bold text-slate-900 dark:text-white"><?= e($activeParent['name']) ?></h2>
                        <p class="text-xs text-slate-500 font-mono">Email: <?= e($activeParent['email']) ?> · Phone: <?= e($activeParent['phone_number']) ?></p>
                    </div>
                </div>
            ...
            <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 p-16 text-center">
                ...
                <h3 class="font-bold text-slate-800 dark:text-white">No Parent Selected</h3>
                <p class="text-xs text-slate-500 mt-1">Please select a parent record from the sidebar registry to review documents.</p>
            </div>
            <?php endif; ?>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (search) {
        search.addEventListener('input', (e) => {
            ...
            const items = document.querySelectorAll('#parentList a');
            items.forEach(item => {
