<?php
$layout    = 'app';
$pageTitle = 'Staff Document Registry';
$breadcrumbs = [['label' => 'Dashboard', 'url' => '/dashboard'], ['label' => 'Documents Workspace', 'url' => '/documents'], ['label' => 'Staff Documents']];
ob_start();
?>
...
<div class="max-w-6xl mx-auto py-2">
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-1 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 p-4 space-y-4">
            <h3 class="font-bold text-slate-800 dark:text-white text-xs uppercase tracking-wider">Staff Directory</h3>
            ...
            <div class="space-y-1 overflow-y-auto max-h-[480px] pr-1" id="staffList">
                <?php foreach ($staffList as $st): ?>
                <a href="?staff_id=<?= $st['id'] ?>" 
                   class="block p-2.5 rounded-xl text-xs transition-all <?= $st['id'] == $staffId ? 'bg-blue-600 text-white font-bold' : 'hover:bg-slate-50 dark:hover:bg-slate-800/40 text-slate-700 dark:text-slate-300' ?>">
                    <div class="truncate"><?= e($st['first_name'] . ' ' . $st['last_name']) ?></div>
                    <div class="text-[10px] opacity-80 font-mono mt-0.5">Code: <?= e($st['employee_code']) ?> · <?= e($st['department']) ?></div>
                </a>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="lg:col-span-3 space-y-6">
            ...
            <div class="p-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 flex items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-blue-500/10 text-blue-500 flex items-center justify-center text-xl font-bold font-mono">
                        <?= strtoupper(substr($activeStaff['first_name'], 0, 1)) ?>
                    </div>
                    <div>
                        <h2 class="text-lg font-bold text-slate-900 dark:text-white"><?= e($activeStaff['first_name'] . ' ' . $activeStaff['last_name']) ?></h2>
                        <p class="text-xs text-slate-500 font-mono">Employee Code: <?= e($activeStaff['employee_code']) ?> · Dept: <?= e($activeStaff['department']) ?></p>
                    </div>
                </div>
                <span class="px-2.5 py-1 rounded-full text-3xs font-extrabold uppercase bg-blue-500/10 text-blue-500">Active Profile</span>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 p-16 text-center">
                ...
                <h3 class="font-bold text-slate-800 dark:text-white">No Employee Selected</h3>
                <p class="text-xs text-slate-500 mt-1">Please select an employee from the sidebar registry to review documents.</p>
            </div>
            <?php endif; ?>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (search) {
        search.addEventListener('input', (e) => {
            ...
            const items = document.querySelectorAll('#staffList a');
            items.forEach(item => {
