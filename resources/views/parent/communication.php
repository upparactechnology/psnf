...
<div class="space-y-6 max-w-4xl">
    ...
    <div class="p-4 rounded-2xl border bg-slate-900/30 border-white/5 flex items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 flex items-center justify-center font-bold text-sm">
                S
            </div>
            <div>
                <h3 class="text-sm font-bold text-white"><?= e($staff['name']) ?></h3>
                <p class="text-[10px] text-slate-500">School Representative (Admin / Teacher)</p>
            </div>
        </div>
        <span class="text-[9px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/10 px-2 py-0.5 rounded-full">
            Online Support
        </span>
    </div>

    <!-- Chat container -->
    <div class="rounded-2xl border border-white/5 bg-slate-900/20 p-5 flex flex-col h-[500px]">
        ...
        <form hx-post="<?= url('parent/communication') ?>"
              ...
              class="flex gap-3 pt-3 border-t border-slate-800/80">
            ...
            <textarea name="message" required x-ref="messageInput"
                      ...
                      class="flex-1 bg-slate-950 border border-slate-800 text-white placeholder-slate-500 rounded-xl py-3 px-4 text-xs focus:outline-none focus:border-brand-500 resize-none"
                      placeholder="Type a message or school query... (Press Enter to send)"></textarea>

            <button type="submit"
                    class="px-5 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs shadow-lg transition-all flex items-center justify-center flex-shrink-0">
                Send
            </button>
        </form>
