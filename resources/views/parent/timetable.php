...
<div class="space-y-6" x-data="{ activeDay: '<?php echo date('l'); ?>'  }">
    ...
    <div class="p-5 rounded-2xl border bg-white dark:bg-slate-900/30 border-slate-200 dark:border-white/5 shadow-sm space-y-4">
        <div>
            <h3 class="text-base font-bold text-slate-800 dark:text-white">Class Timetable</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400">Weekly schedule of learning therapy periods & class sessions</p>
        </div>
    ...
    <div class="space-y-4">
        ...
        <div x-show="activeDay === '<?= $day ?>'" x-transition class="space-y-4">
            ...
            <div class="p-10 rounded-2xl border border-dashed border-slate-350 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/10 text-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-slate-900/20 p-5 flex items-start gap-4 hover:border-slate-350 dark:hover:border-slate-800 transition-colors shadow-sm">
                    <!-- Period Number Badge -->
                    <div class="w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-650 dark:text-indigo-400 flex items-center justify-center font-bold text-sm flex-shrink-0">
                        <?= $idx + 1 ?>
                    </div>
                    <!-- Details -->
                    <div class="space-y-1 min-w-0 flex-1">
                        <h4 class="text-sm font-bold text-slate-800 dark:text-white leading-tight"><?= e($period['subject']) ?></h4>
                        <p class="text-xs text-slate-600 dark:text-slate-400 font-medium"><?= e($period['teacher_name']) ?></p>
                        <div class="flex items-center gap-4 pt-2 text-[10px] text-slate-500 dark:text-slate-500 font-semibold uppercase tracking-wider">
