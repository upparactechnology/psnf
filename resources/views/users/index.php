                ...
                <tr class="hover:bg-slate-800/20 transition-colors group">
                    ...
                    <td class="px-5 py-4 text-right">
                        <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <a href="<?= url('users/'.$u['id'].'/edit') ?>" class="p-1.5 rounded-lg text-slate-500 hover:text-brand-400 hover:bg-brand-900/30 transition-all" title="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            <?php endif; ?>
                            <?php if (has_permission('delete_users') && $u['id'] !== auth_id()): ?>
                            <form method="POST" action="<?= url('users/'.$u['id']) ?>" onsubmit="return confirm('Delete this user?')">
                                <?= \Core\View::csrf() ?>
                                <input type="hidden" name="_method" value="DELETE">
                                <button type="submit" class="p-1.5 rounded-lg text-slate-500 hover:text-red-400 hover:bg-red-900/30 transition-all" title="Delete">
