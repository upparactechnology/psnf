...
ob_start();
$disabilityColors = [
    'ASD'                    => 'bg-blue-900/30 text-blue-400 border-blue-700/30',
    'ADHD'                   => 'bg-orange-900/30 text-orange-400 border-orange-700/30',
    'Down Syndrome'          => 'bg-pink-900/30 text-pink-400 border-pink-700/30',
    'Cerebral Palsy'         => 'bg-purple-900/30 text-purple-400 border-purple-700/30',
    'Dyslexia'               => 'bg-yellow-900/30 text-yellow-400 border-yellow-700/30',
    'Intellectual Disability'=> 'bg-teal-900/30 text-teal-400 border-teal-700/30',
    'Multiple Disabilities'  => 'bg-red-900/30 text-red-400 border-red-700/30',
    'Other'                  => 'bg-slate-700/50 text-slate-300 border-slate-600/30',
];
$statusClasses = [
    ...
    'graduated'  => 'bg-teal-900/30 text-teal-400 border-teal-700/30',
];
?>
...
<div x-data="studentsList()" class="space-y-5">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-xl font-bold text-white">Student Registry</h2>
    ...
    <div class="flex items-center gap-2 overflow-x-auto pb-1">
        ...
        <a href="<?= url('students?' . http_build_query(array_merge($filters, ['status' => $key === 'all' ? '' : $key, 'search' => $search ?? '']))) ?>"
           class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-all <?= $active ? 'bg-brand-600/20 text-brand-400 border border-brand-500/30' : 'text-slate-400 hover:text-slate-300 hover:bg-slate-800/50 border border-transparent' ?>">
            <?= $label ?>
            <span class="text-xs px-1.5 py-0.5 rounded-md <?= $active ? 'bg-brand-700/40' : 'bg-slate-800' ?>"><?= $count ?></span>
        </a>
        <?php endforeach; ?>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3">
        ...
        <select name="disability" id="disability_filter"
                class="bg-slate-900/70 border border-slate-700/60 text-slate-300 rounded-xl py-2.5 px-4 text-sm focus:outline-none focus:border-brand-500 transition-all"
                hx-get="<?= url('students') ?>"
                hx-target="#student-table"
                hx-trigger="change"
                hx-push-url="true">
    ...
    <div id="student-table">
        ...
        <div class="rounded-2xl border border-slate-800/60 bg-slate-900/40 p-16 text-center">
            ...
            <?php if (has_permission('create_students')): ?>
            <a href="<?= url('students/create') ?>" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white" style="background: linear-gradient(135deg, #6366f1, #a855f7);">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                Add First Student
            </a>
            <?php endif; ?>
                    ...
                    <tr class="hover:bg-slate-800/20 transition-colors group">
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <?php endif; ?>
                                <div>
                                    <a href="<?= url('students/' . $student['id']) ?>" class="text-sm font-medium text-white hover:text-brand-300 transition-colors">
                                        <?= e($student['first_name'] . ' ' . $student['last_name']) ?>
                                    </a>
                                    <p class="text-xs text-slate-500"><?= age($student['dob']) ?> · <?= ucfirst($student['gender']) ?></p>
                                </div>
                            </div>
                        </td>
                        <td class="px-5 py-4 hidden md:table-cell">
                            <span class="text-xs font-mono text-slate-400"><?= e($student['admission_number'] ?? '—') ?></span>
                        </td>
                        ...
                        <td class="px-5 py-4 hidden xl:table-cell">
                            <span class="text-xs text-slate-500"><?= e($student['branch_name'] ?? '—') ?></span>
                        </td>
                        <td class="px-5 py-4 text-right">
                            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="<?= url('students/' . $student['id']) ?>" class="p-1.5 rounded-lg text-slate-500 hover:text-white hover:bg-slate-700/50 transition-all" title="View">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <?php if (has_permission('edit_students')): ?>
                                <a href="<?= url('students/' . $student['id'] . '/edit') ?>" class="p-1.5 rounded-lg text-slate-500 hover:text-brand-400 hover:bg-brand-900/30 transition-all" title="Edit">
        ...
        <div class="flex items-center justify-between mt-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="?<?= http_build_query(array_merge($_GET, ['page' => ($current_page ?? 1) - 1])) ?>"
                   class="px-3 py-1.5 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-slate-800 transition-all border border-slate-700/50">
                    ← Prev
                </a>
                <?php endif; ?>
                <?php for ($p = max(1, ($current_page??1) - 2); $p <= min($last_page??1, ($current_page??1) + 2); $p++): ?>
                <a href="?<?= http_build_query(array_merge($_GET, ['page' => $p])) ?>"
                   class="px-3 py-1.5 rounded-lg text-sm transition-all border <?= $p === ($current_page??1) ? 'bg-brand-600/20 text-brand-400 border-brand-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-800 border-slate-700/50' ?>">
                    <?= $p ?>
                </a>
                <?php endfor; ?>
