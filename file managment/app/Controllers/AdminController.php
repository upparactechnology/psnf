...
class AdminController extends Controller {
  ...
  private function buildDashboardData(array $folders, array $resources, array $staff, array $logs, array $folderAssignments, array $resourceAssignments): array {
    ...
    $tree = [];
    $folderSecurityCards = [];
    $buildNode = function(int $folderId, int $depth = 0) use (&$buildNode, &$folderById, &$children, &$resourceCountByFolder, &$folderAssignments, &$storageByFolder, &$folderSecurityCards): array {
      $folder = $folderById[$folderId] ?? ['name' => 'Unknown', 'created_at' => date('Y-m-d H:i:s')];
      $userCount = isset($folderAssignments[$folderId]) ? count($folderAssignments[$folderId]) : 0;
      $fileCount = $resourceCountByFolder[$folderId] ?? 0;
      $status = $userCount === 0 ? 'Private' : ($userCount > 4 ? 'Shared' : 'Restricted');
      $risk = $userCount === 0 && $fileCount > 0 ? 'warning' : ($fileCount > 25 ? 'critical' : 'info');
      $lastUpdated = strtotime((string)($folder['created_at'] ?? 'now'));
      $nodes = [];
      foreach ($children[$folderId] ?? [] as $childId) {
        $nodes[] = $buildNode((int)$childId, $depth + 1);
      }
      $folderSecurityCards[] = [
        'id' => $folderId,
        ...
        'storage' => $storageByFolder[$folderId] ?? 0,
      ];
      return [
        'id' => $folderId,
        'name' => (string)($folder['name'] ?? 'Untitled'),
