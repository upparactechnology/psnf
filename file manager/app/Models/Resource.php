...
class Resource {
  ...
  public static function assignedToStaff(int $staffId): array {
    $allowedFolders = Folder::assignedToStaff($staffId);
    $folderIds = array_column($allowedFolders, 'id');
  ...
  public static function assignedToStaffByFolder(int $staffId, int $folderId): array {
    if (Folder::canStaffAccess($staffId, $folderId)) {
      return self::byFolder($folderId);
    }
  ...
  public static function canStaffAccess(int $staffId, int $resourceId): bool {
    ...
    $st = Database::conn()->prepare($sql);
    $st->execute([$resourceId, $staffId]);
    if ((int)$st->fetch()['c'] > 0) {
      return true;
    }

    $folderId = (int)($resource['folder_id'] ?? 0);
    if ($folderId > 0) {
      return Folder::canStaffAccess($staffId, $folderId);
    }
