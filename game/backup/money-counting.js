...
function checkAnswer() {
  ...
  if (total === target) {
    ...
    fireConfetti();
    playTone("success");
    showToast(`Correct! +${STARS_PER_LEVEL} Stars, +${XP_PER_LEVEL} XP`, "success");
    speak(`Excellent. You made rupees ${target}.`);
    moveToNextLevel();
  } else {
    el.dropZone.classList.add("shake");
    setTimeout(() => el.dropZone.classList.remove("shake"), 350);
    playTone("error");
    showToast(`Try again. You made ₹${total}. Target is ₹${target}.`, "error");
    speak("Try again. Make the exact amount.");
  }
}

function handleTimeExpired() {
  showToast("Time is up. Level reset.", "error");
  playTone("error");
  speak("Time is up. Try this level again.");
  state.timeLeft = START_TIME_SECONDS;
  resetDropped();
  renderTimer();
}
...
function bindControls() {
  ...
  el.resetBtn.addEventListener("click", () => {
    resetDropped();
    showToast("Your money tray was reset.");
  });
